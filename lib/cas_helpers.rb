...
module CasHelpers
  ...
  CAS_CLIENT = CASClient::Client.new(:cas_base_url => 'http://login.hpu.edu.vn', :log => Logger.new(STDOUT), :ticket_store_config => {:storage_dir => ENV['TICKET_STORE_DIR']})
  ...
  def process_cas_login(request, session)
    if request[:ticket] && request[:ticket] != session[:ticket]
      ...
      if st.success
        ...
        puts st.extra_attributes.inspect
      else
